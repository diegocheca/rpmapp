    ...
    public function calcular_porcentajes_personas()
    {
        ...
        foreach ($datos as $key) {
            ...
            if (in_array($key->provincia_id, $provincias_ya_leidas)) {
                ...
                $cantidad_registros++;
                $total_de_personas += doubleval(json_decode($key->datos)->porcentajes_personas->permanentes->profesionales) 
                + doubleval(json_decode($key->datos)->porcentajes_personas->transitorios->profesionales) 
                + doubleval(json_decode($key->datos)->porcentajes_personas->permanentes->otros) 
                + doubleval(json_decode($key->datos)->porcentajes_personas->transitorios->otros) 
                + doubleval(json_decode($key->datos)->porcentajes_personas->permanentes->administrativos) 
                + doubleval(json_decode($key->datos)->porcentajes_personas->transitorios->administrativos) 
                + doubleval(json_decode($key->datos)->porcentajes_personas->permanentes->obreros) 
                + doubleval(json_decode($key->datos)->porcentajes_personas->transitorios->obreros);
            }
        }
