<template>
  ...
  <div
    ...
    :key="indexRow"
    class="flex flex-col bg-purple-600"
  ...
  >
    ...
    <div
      ...
      :key="indexCol"
      class="bg-white rounded shadow-lg p-8 m-4 bg-purple-400"
      :class="col.width"
    >
      ...
      <div class="grid gap-4" :class="[col.columns, col.columnsResponsive]">
        <!-- inputs -->
        <div
          v-for="(item, indexItem) in col.inputs"
          :key="indexItem"
          class="mb-4"
          :class="[item.inputColsSpan]"
        >
          <div class="flex flex-col">
            <label
              :for="item.name"
              class="mb-2 text-md text-grey-darkest flex flex-row"
            >
              {{ item.label }}
            </label>
            ...
            <!-- default -->
            <Field
              v-if="inputsTypes.INPUTS_DEFAULT.indexOf(item.type) > -1"
              :value="item.value"
              :name="item.name"
              :type="item.type"
            ...
            <!-- textarea -->
            <Field
              v-if="item.type == inputsTypes.TEXTAREA"
              :value="item.value"
              :name="item.name"
              :as="item.type"
              class="rounded-md py-2 px-3 text-grey-darkest"
            ...
            <!-- file -->
            <Field
              v-if="item.type == inputsTypes.FILE"
              v-slot="{ field }"
              :type="item.type"
              :name="item.name"
            >
              <div
                v-if="!evaluate"
                class="w-full h-full border-2 border-red-500 rounded-md"
              >
                <DragAndDropFile
                  v-bind="field"
                  :accept="item.accept"
            ...
            <template v-if="item.type == inputsTypes.LIST">
              <fieldset
                class="px-5 my-3 bg-blue-100 rounded-lg"
                v-for="(element, indexElement) in item.childrens"
                :key="indexElement"
              >
                <div v-if="!evaluate" class="btn-close-row">
                  <svg
                    ...
                    viewBox="0 0 20 20"
                    fill="#EF4444"
                    @click="removeRowDynamic(item, indexElement)"
                  >
                    <path
                      fill-rule="evenodd"
                ...
                >
                  ...
                  >
                    ...
                    <label
                      v-if="ele.label"
                      :for="ele.label"
                      class="mb-2 text-md text-grey-darkest"
                      >{{ ele.label }} :</label
                    >
                        ...
                        >
                          ...
                          >
                            ...
                            <div class="relative">
                              ...
                              <input
                                :type="ele.type"
                                :name="`${item.label}[${indexElement}].${a[indexElement2].name}`"
                                :id="ele.name"
                                class="sr-only"
                                v-model="ele.value"
                                v-bind="field"
                    ...
                    <Field
                      v-if="
                        inputsTypes.INPUTS_DEFAULT.indexOf(
                          a[indexElement2].type
                        ) > -1
                      "
                      :value="a[indexElement2].value"
                      :name="`${item.label}[${indexElement}].${a[indexElement2].name}`"
                      :type="a[indexElement2].type"
                      class="rounded-md py-2 px-3 text-grey-darkest"
                      :disabled="evaluate ? true : false"
                    />
                    ...
                    <!-- textarea -->
                    <Field
                      v-if="ele.type == inputsTypes.TEXTAREA"
                      :value="ele.value"
                      :name="`${item.label}[${indexElement}].${a[indexElement2].name}`"
                      :as="ele.type"
                      class="rounded-md py-2 px-3 text-grey-darkest"
                    ...
                    <!-- file -->
                    <Field
                      v-if="ele.type == inputsTypes.FILE"
                      v-slot="{ field }"
                      :type="ele.type"
                      :name="`${item.label}[${indexElement}].${a[indexElement2].name}`"
                    >
                    ...
                    >
                      <VueMultiselect
                        v-bind="field"
                        v-model="element[indexElement2].value"
                        :name="`${item.label}[${indexElement}].${a[indexElement2].name}`"
                        :ref="`${indexElement}-${a[indexElement2].name}`"
                        :id="{
                          ele,
                          id: `${indexElement}`,
                        }"
                        ...
                        :close-on-select="ele.closeOnSelect"
                        :searchable="ele.sercheable"
                    ...
                    <!-- review -->
                    <div
                      class="grid grid-row-2 p-4 rounded-lg"
                      :class="[
                        values[
                          `ele.observation-${indexElement2}.options[0].name`
                        ] != 'rechazado'
                          ? 'bg-blue-200'
                          : 'bg-red-200',
                      ]"
                      v-if="evaluate && ele.observation"
                    >
                      <div class="w-full flex flex-wrap">
                        <span class="w-full text-gray-700"> Correcto? </span>
                        <!-- inputs radio -->
                        <div
                          v-for="(
                            obs, index
                          ) in `ele.observation-${indexElement2}.options`"
                          :key="index"
                        >
                          <label>
                            <Field
                        ...
                        <ErrorMessage
                          ...
                          :name="`ele.observation-${indexElement2}.options[0].name`"
                        />
                      </div>
                      <!-- textarea -->
                      ...
                      >
                        <label
                          ...
                          class="mb-2 text-md text-grey-darkest"
                          >{{
                            `ele.observation-${indexElement2}.comment.label`
                          }}
                          :</label
                        >
                        <Field
                          :name="`ele.observation-${indexElement2}.comment.name`"
                          :as="`ele.observation-${indexElement2}.comment.type`"
                          class="rounded-md py-2 px-3 text-grey-darkest"
                          :value="`ele.observation-${indexElement2}.comment.value`"
                        />
                        <ErrorMessage
                          class="text-red-500"
                          :name="`ele.observation-${indexElement2}.comment.name`"
            ...
            <!-- review -->
            <div
              class="grid grid-rows-2 grid-flow-col p-4 mt-5 rounded-lg"
              :class="[
                values[item.observation.options[0].name] != 'rechazado'
                  ? 'bg-blue-200'
                  : 'bg-red-200',
              ]"
              v-if="
                evaluate && item.observation && item.type != inputsTypes.LIST
              "
            >
              <div class="w-full flex flex-wrap">
                <span class="w-full text-gray-700"> Correcto? </span>
                <!-- inputs radio -->
                <div
                  v-for="(obs, index) in item.observation.options"
                  :key="index"
                >
                  <label>
                    <Field
                ...
                <ErrorMessage
                  class="w-full text-red-500"
                  :name="item.observation.options[0].name"
                />
              </div>
              <!-- textarea -->
              <div
                v-show="values[item.observation.options[0].name] == 'rechazado'"
                class="w-full flex flex-col transition duration-500 ease-in"
              >
                <label :for="item.name" class="mb-2 text-md text-grey-darkest"
                  >{{ item.observation.comment.label }} :</label
                >
                <Field
                  :name="item.observation.comment.name"
                  :as="item.observation.comment.type"
                  class="rounded-md py-2 px-3 text-grey-darkest"
                  :value="item.observation.comment.value"
                />
                <ErrorMessage
                  class="text-red-500"
                  :name="item.observation.comment.name"
  ...
  methods: {
    ...
    getSelectOptions(value, element) {
      ...
      for (let index = 0; index < elementChange.length; index++) {
        ...
        this.$refs[`${element.id}-${elementDepends}`].options.splice(
          0,
          this.$refs[`${element.id}-${elementDepends}`].options.length
        );
    ...
    async getAsyncOptionsSelect(value, element) {
      // console.log(element);
      if (!element || !element.inputDepends || element.inputDepends.length == 0)
        return;
      ...
      for (let index = 0; index < elementChange.length; index++) {
        if (value) {
          ...
          try {
            ...
            for (
              let index = 0;
              index < element.inputClearDepends.length;
              index++
            ) {
              const clear = element.inputClearDepends[index];
              this.$refs[clear].options.splice(
                0,
                this.$refs[clear].options.length
              );
