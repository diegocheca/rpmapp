  ...
  <div>
    ...
    <div class="flex items-stretch w-full relative">
      ...
      <input
        ...
        :disabled="evaluacion || $props.cuit_disable"
    ...
    <div
      v-if="
        evaluacion || cuit_correccion_mostrar || $props.mostrar_evaluacion_adm
      "
    >
      <!-- <div class="w-full md:w-1/3 px-3">
        ...
        <div class="mt-2">
          <label class="inline-flex items-center">
            <input
              type="radio"
              :disabled="$props.cuit_correccion_desactivar"
              class="form-radio h-5 w-5 text-green-600"
              name="name_cuit_correcto"
              v-model="cuit_correcto"
              value="true"
              v-on:change="actaulizar_variable_cuit(true)"
            />
          ...
          <label class="inline-flex items-center ml-6">
            <input
              ...
              v-model="cuit_correcto"
              value="false"
              v-on:change="actaulizar_variable_cuit(false)"
            />
          ...
          <label class="inline-flex items-center ml-6">
            <input
              ...
              v-model="cuit_correcto"
              value="nada"
              v-on:change="actaulizar_variable_cuit('nada')"
            />
      ...
      <div v-show="!cuit_correcto_local" class="w-full md:w-2/3 px-3">
        ...
        <p v-bind:class="clase_cartel_nota_evaluacion_cuit_text_area">
          {{ cartel_nota_evaluacion_cuit_text_area }}
        </p>
      </div> -->
      <SeccionEvaluacion
        :correccion_desactivar="$props.cuit_correccion_desactivar"
        :name_correcto="'name_cuit_correcto'"
        :correcto="cuit_correcto"
        :obs_observacion="observacion_correcto_local"
        v-on:change_correcto="actaulizar_variable_cuit($event)"
        v-on:change_obs="actaulizar_contenido_text_area_cuit($event)"
      >
      </SeccionEvaluacion>
    </div>
    <div
...
export default {
  ...
  props: [
    ...
    "testing",
    "cuit_disable",
    "cuit_correccion_mostrar",
    "cuit_correccion_desactivar",
  ...
  data() {
    return {
      ...
      observacion_correcto_local: this.$props.obs_cuit,
      cuit_valido_local: this.$props.email_valido,
      clase_text_area_cuit:
      ...
      clase_cartel_nota_evaluacion_cuit_text_area:
  ...
  methods: {
    ...
    actaulizar_contenido_text_area_cuit(value) {
      this.observacion_correcto_local = value;
      this.$emit("changeobscuit", value);
      //   if (this.$props.obs_cuit.length <= 2) {
      //     this.clase_text_area_cuit =
      //       "appearance-none block w-full bg-gray-200 text-gray-700 border border-red-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white";
      //     this.cartel_nota_evaluacion_cuit_text_area =
      //       "Observacion Incorrecta - debe ser mayor a 2 carcteres";
      //     this.clase_cartel_nota_evaluacion_cuit_text_area =
      ...
      //   if (
      //     this.$props.obs_cuit !== "" &&
      //     this.$props.obs_cuit.length <= 15 &&
      //     this.$props.obs_cuit.length >= 3
      //   ) {
      //     this.clase_text_area_cuit =
      //       "appearance-none block w-full bg-gray-200 text-gray-700 border border-green-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white";
